...
| `TDR-02` Prototype API compatibility | M03-D06 | Make a clean `/api/v1` cutover because the current API is an unauthenticated prototype. Do not maintain `X-Org-ID`, JWT, or unversioned compatibility routes. | [Approved route inventory and cutover plan](implementation/m03/d06-versioned-route-cutover.md). |
...
| `TDR-06` Browser identity architecture | M03-D01 | **Closed:** use bounded local email/password verification and revocable opaque BFF sessions. Preserve accounts; do not make OIDC an M03 dependency. | [ADR-005 — Local password authentication with BFF sessions](decisions/adr-005-local-password-bff-sessions.md). |
...
- `TDR-03` and `TDR-07` require explicit security approval because they govern
  cryptographic key material or external data transfer. TDR-06 is closed by its
